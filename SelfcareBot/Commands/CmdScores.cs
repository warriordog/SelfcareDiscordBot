            ...
            using (_logger.BeginScope($"CmdScores.ScoresCommand@{ctx.Message.Id.ToString()}"))
            {
                try
                {
                    ...
                    if (leaderboard.Any())
                    {
                        ...
                        var maxScoreDecimals = (int) Math.Floor(
                            Math.Log10(
                                leaderboard[0]
                                    .Score
                            )
                        ...
                        leaderboardLines.AddRange(leaderboard.Select(entry => $"#{entry.Rank.ToString().PadLeft(maxRankDecimals)}:\t{entry.Score.ToString().PadLeft(maxScoreDecimals)}\t{entry.Username}#{entry.Discriminator}"));
                    }
                    else
                    {
